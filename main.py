...
from database.models import Users, Articles, ArticleViews
...
def article_detail(article_id):
    ...
    user = Users.select().where(Users.username == logged)
    ArticleViews.create(
        article=data,
        user=user
    )
    views = ArticleViews.select().where(ArticleViews.article_id == article_id)
    views = model_converter(views)
    checked_id = []
    all_views = []
    for i in views:
        if i["user"]["id"] not in checked_id:
            all_views.append(i)
            checked_id.append(i["user"]["id"])
    Articles.update(views=len(all_views)).where(Articles.id == article[0]["id"]).execute()
    data = Articles.select().where(Articles.id == article_id)
    article2 = model_converter(data)

    return render_template('article_detail.html', article=article2[0], admin=admin, logged=logged)
